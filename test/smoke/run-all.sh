...
#   2. Runs every smoke case under `cases/0[1-9]-*.sh` and `cases/1[0-8]-*.sh`
...
log ""
log "===== running cases 01-18 (cleanup before each) ====="

declare -a RESULTS
PASS=0
FAIL=0
BLOCKED=0
CASE_GLOB=( "${SMOKE_DIR}"/cases/0[1-9]-*.sh "${SMOKE_DIR}"/cases/1[0-8]-*.sh )
